...
from backtesting.sensitivity import tabla_sensibilidad
...
def main():
    ...
    for lam in LAMBDAS:
        ...
        pd.DataFrame(registros).set_index(["param_value", "perfil"]).to_csv(out_path)
        print(f"  lambda={lam} guardado en {out_path}", flush=True)

    df_sens = pd.DataFrame(registros).set_index(["param_value", "perfil"])
