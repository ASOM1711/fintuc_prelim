"""
Test para elegir max_weight usando ventana rolling de training de 5 años.

Evalua max_weight = 5%, 10%, 15%, 20%, 25%.
Usa lambda=0 y full_invest=True.
"""
...
FULL_INVEST = True
TRAIN_YEARS = 5
MAX_WEIGHTS = [0.025, 0.05, 0.10, 0.15, 0.20]
...
def _run_metricas_anuales(price_returns, div_yields, market_caps, end_year: int, out_dir: Path) -> pd.DataFrame:
    registros = []
    ...
    for max_weight in MAX_WEIGHTS:
        ...
        pd.DataFrame(registros).to_csv(
            out_dir / "max_weight_decision_train5_metricas_anuales_parcial.csv",
            index=False,
        )
...
def _run_error_predictivo(price_returns, market_caps, out_dir: Path) -> pd.DataFrame:
    registros = []
    for max_weight in MAX_WEIGHTS:
        print(f"\nError predictivo max_weight={max_weight:.0%}, train_years={TRAIN_YEARS}", flush=True)
        ...
        pd.concat(registros, ignore_index=True).to_csv(
            out_dir / "max_weight_decision_train5_error_predictivo_parcial.csv",
            index=False,
        )
...
def main():
    ...
    metricas = _run_metricas_anuales(price_returns, div_yields, market_caps, end_year, out_dir)
    metricas.to_csv(out_dir / "max_weight_decision_train5_metricas_anuales.csv", index=False)

    error = _run_error_predictivo(price_returns, market_caps, out_dir)
    error.to_csv(out_dir / "max_weight_decision_train5_error_predictivo.csv", index=False)
